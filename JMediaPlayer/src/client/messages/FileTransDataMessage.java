...
/**
 * Class containing information on the messages that deal with
 * sending actual file data between clients.
 * 
...
public class FileTransDataMessage extends FileTransferMessage {
	 ...
	 * information to be sent to the server.  The message is ready to be
	 * sent to the server through its <code>sendMessage</code> method.
	 * 
	 ...
	 * @param a_numOfTotSeg <code>int</code>, the total number of data 
	 * 			segments for the file
	 * @param a_sizeOfCurSeg <code>int</code>, the size(in bytes) of the 
	 * 			current data segment
	 * @param a_segment <code>byte[]</code>, the file data for the current 
	 * 			data segment
	...
	public FileTransDataMessage(MessageHeader a_header, int a_transferID,
		...
		if( setDataSegment( a_segment ) == false ){
			throw new Exception(
					"ERROR: Invalid data segment size(SEGMENT)");
		}
	}
	 ...
	 * </br>the size of the receiver's username and the receiver's name
